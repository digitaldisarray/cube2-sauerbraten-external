...
Vec2 CalcAngle(Vec3 src, Vec3 dst) {
    ...
    float deltaX = (src.x) - (dst.x);
    float deltaY = (src.y) - (dst.y);

    if ((dst.x) > (src.x) && (dst.y) <= (src.y))
    {
        angles.x = atanf(deltaX / deltaY) * -180.0f / 3.141593;
    }

    else if ((dst.x) >= (src.x) && (dst.y) > (src.y))
    {
        angles.x = atanf(deltaX / deltaY) * -180.0f / 3.141593 + 180.0f;
    }
    ...
    {
        angles.x = atanf(deltaX / deltaY) * -180.0f / 3.141593 + 360.0f;
    }

    float deltaZ = dst.z - src.z;
    float dist = sqrt(
        pow(double(dst.x - src.x), 2.0) +
        pow(double(dst.y - src.y), 2.0) +
        pow(double(dst.z - src.z), 2.0));

    angles.y = asinf(deltaZ / dist) * -180.0f / 3.141593;
